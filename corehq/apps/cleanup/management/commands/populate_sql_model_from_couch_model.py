...
Run the following commands to run the migration and get up to date:
    ...
    def _migrate_docs(self, docs, logfile, fixup_diffs):
        ...
        if creates or updates:
            ...
            ignore_conficts = not couch_class._migration_get_submodels()
            with transaction.atomic(), disable_sync_to_couch(sql_class):
                sql_class.objects.bulk_create(creates, ignore_conflicts=ignore_conficts)
                for sub_class, sub_creates in self._group_submodels(creates):
    ...
    def _group_submodels(self, creates):
        combined = defaultdict(list)
        for spec in self.sql_class()._migration_get_couch_model_class()._migration_get_submodels():
            for obj in creates:
                combined[spec.sql_class].extend(obj._new_submodels[spec.sql_class][0])
